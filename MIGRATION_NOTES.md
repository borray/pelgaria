...
### Доступ суперадмина к новым функциям (исправление, обратносовместимо)
- Проблема: историческая роль «Глава государства» в проде создана до появления права
  `system.superadmin`, поэтому её токен его не содержал. `requirePermission` пропускает только
  по `system.superadmin` (без fallback по имени роли) → новые разделы (в т.ч. «Проектные
  инициативы») были недоступны Председателю и на сервере, и в меню.
- Решение: `effectivePermissions(role)` в `middleware/permissions.ts` — для суперадмин-роли
  (право `system.superadmin` ИЛИ имя «Глава государства») гарантированно добавляет
  `system.superadmin` в выдаваемые клиенту/в токен права. Применено в `auth.ts` (login, refresh,
  change-login, `/me`). На фронте `hasPermission` пропускает обладателя `system.superadmin`.
- **Данные:** правка только в рантайме выдачи прав, схема и записи ролей не меняются. Достаточно
  заново войти (или дождаться refresh токена), чтобы Председатель увидел все разделы.
